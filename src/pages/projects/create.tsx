...
import { techStackOptions, categoryOptions, difficultyOptions } from "@/lib/seedData"
...
const STEPS = [
    { id: 1, title: "Project Basics", description: "Title, description & category", icon: <Code className="w-5 h-5" /> },
    { id: 2, title: "Technical Details", description: "Tech stack & difficulty", icon: <Settings className="w-5 h-5" /> },
    { id: 3, title: "Features & Goals", description: "What you'll build", icon: <Target className="w-5 h-5" /> },
    { id: 4, title: "Team & Collaboration", description: "Who you need", icon: <Users className="w-5 h-5" /> }
]

const timezones = [
    "UTC", "EST", "PST", "GMT", "CET", "JST", "IST", "AEST", "CST"
]
...
export default function CreateProjectPage() {
    ...
    const addToArray = (field: string, value: string) => {
        if (value && !formData[field as keyof ProjectForm].includes(value)) {
            setFormData((prev: ProjectForm) => ({
                ...prev,
                [field]: [...(prev[field as keyof ProjectForm] as string[]), value]
            }))
        }
    }
    ...
    const removeFromArray = (field: string, valueToRemove: string) => {
        setFormData((prev: ProjectForm) => ({
            ...prev,
            [field]: (prev[field as keyof ProjectForm] as string[]).filter(item => item !== valueToRemove)
        }))
    }

    const addCustomItem = (field: string, customValue: string, setCustomValue: (value: string) => void) => {
        if (customValue.trim()) {
        ...
        switch (step) {
            case 1:
                return formData.title.length >= 3 && 
                       ...
                       formData.category !== ""
            case 2:
                return formData.techStack.length >= 1 && 
                       formData.difficulty !== ""
            case 3:
            ...
            <main className="flex-1 container mx-auto px-4 py-8">
                ...
                <div className="text-center mb-8">
                    ...
                    <motion.p
                        ...
                        Share your project idea and find the perfect developers to build it with you
                    </motion.p>
                </div>
                        ...
                        <AnimatePresence mode="wait">
                                ...
                                <motion.div
                                    ...
                                    <div>
                                        <label className="block text-white font-medium mb-2">
                                            ...
                                            Tech Stack * (Select technologies you'll use)
                                        </label>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex space-x-2">
                                                <input
                                                    ...
                                                    onKeyPress={(e) => e.key === "Enter" && addCustomItem("techStack", customTech, setCustomTech)}
                                                />
                                                <button
                                                    onClick={() => addCustomItem("techStack", customTech, setCustomTech)}
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-4">
                                            {techStackOptions.slice(0, 16).map((tech) => (
                                                <button
                                                    key={tech}
                                        ...
                                        {formData.techStack.length > 0 && (
                                            <div>
                                                <p className="text-gray-300 text-sm mb-2">Selected Technologies:</p>
                                ...
                                <motion.div
                                    ...
                                    <div>
                                        <label className="block text-white font-medium mb-2">
                                            ...
                                            Key Features * (What will your project include?)
                                        </label>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex space-x-2">
                                                <input
                                                    ...
                                                    className="flex-1 bg-white/5 border border-white/20 rounded-xl px-4 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-purple-500"
                                                    onKeyPress={(e) => e.key === "Enter" && addCustomItem("features", customFeature, setCustomFeature)}
                                                />
                                                <button
                                                    onClick={() => addCustomItem("features", customFeature, setCustomFeature)}
                                                    className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-xl transition-colors">
                                                    <Plus className="w-4 h-4" />
                                                </button>
                                            </div>
                                        ...
                                        {formData.features.length > 0 && (
                                            <div>
                                                <p className="text-gray-300 text-sm mb-2">Project Features:</p>
                                                <div className="space-y-2">
                                    ...
                                    <div>
                                        <label className="block text-white font-medium mb-2">
                                            ...
                                            Learning Outcomes (What will team members learn?)
                                        </label>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex space-x-2">
                                                <input
                                                    ...
                                                    className="flex-1 bg-white/5 border border-white/20 rounded-xl px-4 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-purple-500"
                                                    onKeyPress={(e) => e.key === "Enter" && addCustomItem("learningOutcomes", customOutcome, setCustomOutcome)}
                                                />
                                                <button
                                                    onClick={() => addCustomItem("learningOutcomes", customOutcome, setCustomOutcome)}
                                                    className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-xl transition-colors">
                                                    <Plus className="w-4 h-4" />
                                                </button>
                                            </div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {formData.learningOutcomes.map((outcome) => (
                                                    <span
                                                        ...
                                                        <span>{outcome}</span>
                                                        <button
                                                            onClick={() => removeFromArray("learningOutcomes", outcome)}
                                                            className="hover:bg-white/20 rounded-full p-0.5">
                                                            <X className="w-3 h-3" />
                                                        </button>
                                                    </span>
                                    ...
                                    <div>
                                        <label className="block text-white font-medium mb-2">
                                            Required Skills * (What skills do team members need?)
                                        </label>
                            ...
                            {currentStep === 4 && (
                                <motion.div
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                <option value="Weekend">Weekend (5-10 hrs/week)</option>
                                    ...
                                    <div>
                                        <label className="block text-white font-medium mb-2">
                                            Looking For * (What kind of developers do you need?)
                                        </label>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex space-x-2">
                                                <input
                                                    ...
                                                    className="flex-1 bg-white/5 border border-white/20 rounded-xl px-4 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-purple-500"
                                                    onKeyPress={(e) => e.key === "Enter" && addCustomItem("lookingFor", customLookingFor, setCustomLookingFor)}
                                                />
                                                <button
                                                    onClick={() => addCustomItem("lookingFor", customLookingFor, setCustomLookingFor)}
                                                    className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-xl transition-colors">
                                                    <Plus className="w-4 h-4" />
                                                </button>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-4">
                                            {lookingForOptions.map((option) => (
                                                <button
                                                    key={option}
                                                    onClick={() => addToArray("lookingFor", option)}
                                                    disabled={formData.lookingFor.includes(option)}
                                                    className={`
                                                        p-2 rounded-lg text-sm transition-all duration-200
                                        ...
                                        {formData.lookingFor.length > 0 && (
                                            <div>
                                                <p className="text-gray-300 text-sm mb-2">Looking for:</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {formData.lookingFor.map((role) => (
                                                        <span
                                                            ...
                                                            <span>{role}</span>
                                                            <button
                                                                onClick={() => removeFromArray("lookingFor", role)}
                                                                className="hover:bg-white/20 rounded-full p-0.5">
                                                                <X className="w-3 h-3" />
                                                            </button>
                                                        </span>
