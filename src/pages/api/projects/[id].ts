...
export default async function handler(
    ...
    try {
        ...
        switch (req.method) {
            ...
            case "PATCH":
                ...
                if (
                    ...
                    (!Array.isArray(updateData.techStack) ||
                        updateData.techStack.length === 0 ||
                        updateData.techStack.length > 15)
                ) {
                    return res
                        .status(400)
                        .json({
                ...
                if (
                    updateData.teamSize &&
                    (updateData.teamSize < 1 || updateData.teamSize > 10)
                ) {
                    return res
                        .status(400)
                        .json({ error: "Team size must be between 1 and 10" })
                }
        ...
        console.error(
            `Error handling ${req.method} request for project ${id}:`,
            error
        )
